...
from runners.diffusion_mcj import Diffusion
...
def parse_args_and_config():
    ...
    parser.add_argument(
        "--config", type=str, default='marmousi.yml', help="Path to the config file"
    )
    ...
    parser.add_argument("--fid", default=False)
    parser.add_argument("--interpolation", default=False)
    parser.add_argument(
        "--resume_training", default=False, help="Whether to resume training"
    )
    parser.add_argument(
        "-i",
        "--image_folder",
    ...
    parser.add_argument("--use_pretrained", default=False, help='Automatically load the model according to the dataset')
    parser.add_argument(
        "--generalized",
        type=str,
        default="ddpm_noisy", #
    ...
    parser.add_argument("--sequence", default=True)
    ...
    parser.add_argument('--cropped_patch_size', default=(32, 32), type=int,
                        help='patch size of randomly cropped patch afer aug')
    parser.add_argument('--cropped_data_num', default=100000, type=int,
                        help='the num of the randomly cropped patch afer aug')
    ...
    if not args.test and not args.sample:
        ...
        if args.sample:
            os.makedirs(os.path.join(args.exp, "image_samples", args.doc), exist_ok=True)
            args.image_folder = os.path.join(
                args.exp, "image_samples", args.image_folder, args.doc
            )
...
def main():
    ...
    # args.resume_ckpt_id = 290000
    args.siesmic_dir = '/home/shendi_mcj/datasets/seismic/marmousi/marmousi35'
    ...
    args.sample_type= "generalized" # generalized or ddpm_noisy

    config.model.ch_mult = [1, 1, 2, 2, 4, 4]
    config.training.snapshot_freq = 10000
    config.diffusion.beta_schedule = 'pow'  # 'cosine' "linear" "const" "jsd"  "sigmoid" 'pow'
    config.training.snapshot_freq = 10000
    ...
    args.doc='marmousi_pow3'# MmsSegyopenf_cos marmousi
    # args.exp ='/home/shendi_mcj/code/Reproducible/ddim-main/exp/logs/MmsSegyopenf_pow3'
    config.data.image_size = 128
    config.sampling.ckpt_id = 300000
    config.model.ch_mult = [1, 1, 2, 2, 4, 4]  # (1, 2, 3, 4)   #[1, 2, 2, 3, 4] [1, 1, 2, 2, 4, 4]
    config.sampling.num_samples=100
